 ...
 * Program Description: Keeps track of heap in the from of an arraylist where the largest value is at the top of the heap
 * 
 * @param <E>
 */
public class MaxHeap<E> extends Heap
{
	...
	*/
	public MaxHeap(Comparator<E> comp, int cap)
	{
		if (cap < 1) 
		{
	...
	public int compare(Object parent, Object child)
	{
		...
		if(comparator != null)
		{
			return comparator.compare((E)parent, (E)child);//This will give us an opposite value
		}
		else
		{
			return ((Comparable<E>)parent).compareTo((E) child);//This will give us an opposite value
		}
	...
	public Object poll()
	{
		...
		do
		{
			...
			maxChild = leftChild; // Assume leftChild is smaller.
			...
			if (compare(theData.get(parent), theData.get(maxChild)) < 0) //still have to check for the first time
			{
				swap(parent, maxChild);
				parent = maxChild;
			} 
		}
		while(compare(theData.get(parent), theData.get(maxChild)) < 0);
